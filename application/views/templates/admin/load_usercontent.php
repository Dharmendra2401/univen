<table id="example1" class="table table-striped table-bordered " width='100%'>
                            ...
                            <tbody>
                                <tr <?php $i=1;  foreach($row as $index)  {?>>
                                    <td><?php echo $i; ?></td>
                                    <td class="text-center"><?php if($index['id']=='1'){ echo "<label class='text-white bg-success btn-sm'>Aspirant</label>";}elseif($index['id']=='2'){echo "<label class='text-white bg-success btn-sm'>Employer<label>";}elseif($index['id']=='3'){echo "<label class='text-white bg-success btn-sm'>Trainer<label>";}else{echo "<label class='text-white bg-success btn-sm'>Merchant<label>";} ?> 
                                   </td>
                                    <td class="text-uppercase"><img src="<?php echo base_url().'uploads/user/'.$index['image'];?>" width="100px">
                                   </td>
                                   <td>
                                   <?php echo date('d-M-Y H:i:s',strtotime($index['record_inserted_dttm'])); ?>
                                    ...
                                    <a href="#" title="edit" data-toggle="modal" data-target="#update" onclick="return update('<?php echo base64_encode($index['id']); ?>','<?php echo base64_encode($index['image']); ?>','<?php echo base64_encode($index['content']); ?>');" class="rounded btn btn-sm btn-success"><i class="fas fa-pencil-alt"></i></a>
