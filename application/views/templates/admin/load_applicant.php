<table id="example1" class="table table-striped table-bordered " width='100%'>
                            ...
                            <?php $i=1;foreach($row as $index)  { 
                                if($index['email_verified']=='Y'){ 
                                 ...
                                 if(($index['admin_approval']=='Yes')||($index['admin_approval']=='yes')){ 
                                ...
                                <tr class="row<?php echo $i; ?>">
                                    ...
                                    <a href="<?php echo base_url();?>admin/view_applicant?token=<?php echo base64_encode($index['id'])?>" title="View details" class="rounded btn btn-sm btn-success"><i class="fas fa-eye"></i></a> <br> 

                                    <a href="#" title="delete" data-toggle="modal"  onclick="return btnclickdelete('<?php echo $index['id']; ?>','user');" class="rounded btn btn-sm btn-danger"><i class="fas fa-trash"></i></a>  <?php if(($index['admin_approval']=='Yes')||($index['admin_approval']=='yes')){ ?>
                                    <a href="#" title="Click here to disapprove" data-toggle="modal"  onclick="return unverify('<?php echo $index['id']; ?>','No','user');" class="rounded btn btn-sm btn-success"><i class="fas fa-check"></i></a>
                                    <?php }else{ ?>
                                    <a href="#" title="Click here to approve" data-toggle="modal"  onclick="return verify('<?php echo $index['id']; ?>','Yes','user');" class="rounded btn btn-sm btn-danger"><i class="fas fa-times"></i></a>
                                    <?php } ?></td>
                                </tr>
                                <?php $i++;} ?>
